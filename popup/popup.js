...
// Save API key automatically when document content is loaded
document.addEventListener('DOMContentLoaded', function () {
    const apiKey = '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'; // Predefined API key
    ...
    // Setup UI to display and manage the API key
    const apiKeyInput = document.getElementById('apiKey');
    const deleteButton = document.getElementById('delete-button');
    const statusMessage = document.getElementById('status-message');

    apiKeyInput.value = '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';  // Set API key input field value

    deleteButton.addEventListener('click', function () {
        chrome.storage.local.remove(['apiKey'], function () {  // Remove API key from local storage
            statusMessage.textContent = 'API key deleted successfully!';  // Update status message
            apiKeyInput.value = '';
            setTimeout(function () {
                statusMessage.textContent = '';  // Clear status message after 2 seconds
            }, 2000);
        });
    });

    // Handle localization for UI elements (assumes localization setup is done)
